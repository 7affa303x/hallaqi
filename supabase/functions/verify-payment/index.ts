...
serve(async (req) => {
  ...
  try {
    ...
    if (!sessionId) {
      return json(req, { error: 'sessionId is required' }, 400)
    }

    if (provider !== 'stripe') {
      return json(req, { error: `Provider "${provider}" verification not implemented` }, 400)
    }
    ...
    const { data: payment, error: paymentError } = await supabase
      ...
      .eq('provider', 'stripe')
      .single()
    if (paymentError || !payment?.booking_id) return json(req, { error: 'Payment not found' }, 404)
    const { data: booking } = await supabase
      .from('bookings')
      .select('client_id')
      .eq('id', payment.booking_id)
      .single()
    if (!booking || booking.client_id !== user.id) return json(req, { error: 'Not authorized' }, 403)
    ...
    if (bookingId !== payment.booking_id) return json(req, { error: 'Payment metadata mismatch' }, 409)

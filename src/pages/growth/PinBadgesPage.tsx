...
/** Pick up to 8 unlocked badges to show on profile. */
export default function PinBadgesPage() {
  const { themeConfig, goBack } = useApp();
  const { snapshot, pinBadges } = useGrowth();
  const unlocked = useMemo(
    () => snapshot.badges.filter(b => !b.locked),
    [snapshot.badges],
  );
  const initialPinned = useMemo(
    () => snapshot.pinnedBadges.map(b => b.id),
    [snapshot.pinnedBadges],
  );
  ...
  return (
    <GrowthPageShell
      title="تثبيت الشارات"
      subtitle={`اختر حتى ${MAX_PINNED_BADGES} شارات للعرض في البروفايل`}
      badge={`${selected.length}/${MAX_PINNED_BADGES}`}
    >
      <p className="text-[11px] mb-4 leading-relaxed" style={{ color: themeConfig.colors.textMuted }}>
        اضغط على الشارات المفتوحة لتثبيتها. الشارات المثبتة تظهر في بطاقة «شارات مثبتة» على البروفايل.
      </p>
        ...
        {unlocked.map((badge) => {
          ...
          return (
            <button
              ...
              disabled={full}
              className="relative flex flex-col items-center gap-1.5 rounded-2xl p-3 border text-center transition-all"
              style={{
            ...
            >
              ...
              <span className="text-[10px] font-bold leading-tight line-clamp-2" style={{ color: themeConfig.colors.text }}>
                {badge.name}
              </span>
            </button>
          );
      ...
      {unlocked.length === 0 && (
        <p className="text-center text-sm py-8" style={{ color: themeConfig.colors.textMuted }}>
          لا توجد شارات مفتوحة بعد. أكمل المهمات والإنجازات لفتح الشارات.
        </p>
      )}
      ...
      <button
        ...
        className="w-full h-12 rounded-2xl text-sm font-bold text-white flex items-center justify-center gap-2 disabled:opacity-50"
        style={{ backgroundColor: themeConfig.colors.primary }}
      >
        <Check size={16} />
        حفظ التثبيت ({selected.length})
      </button>
    </GrowthPageShell>
  );
}
